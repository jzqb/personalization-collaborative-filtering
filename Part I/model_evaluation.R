...
##Cross-validation evaluation scheme on training set to tune parameters and select best models
# Parameters
...
#   threshold: jokes with a rating higher than this threshhold will be considered positive
...
######################################################################
#Compare models (use default neighborhood size)
######################################################################
...
train_eval <- function(proportion, given = 15, method, param){
  split <- train_test_split(ratings, train_proportion = proportion)
  train_data <- split$train_data
  training_sparse <- as(as.matrix(train_data), 'realRatingMatrix')
...
#Item-based Pearson
size_err_IB <- lapply(sample_prop, train_eval, method = "IBCF", param = list(k = 10, method = 'pearson'))
...
size_err_all <- rbind(size_err_IB, size_err_UB)


######################################################################
#Error vs. items_to_keep
######################################################################
...
keep_err_IB <- lapply(keep, train_eval, proportion = .2, method = "IBCF", param = list(k = 10, method = 'pearson'))
...
keep_err_all <- rbind(keep_err_IB, keep_err_UB)
