...
* The models are summarized in the following section, and we see that Factorization Machine outperform any single model. We compared the models against the random accuracy [predicting 1 for every user, which translates to recommending all songs to every user] and see that the algorithms are better than random prediction.
* In content based recommender, we see that increasing the nearest neighbors in knn, increased the accuracy of the model, it is just over SVD performance. However, the cost of building content based recommender is very high, as each user had its own model that was bult based on the features of the songs preferred by him/her.
* Upon hybridization, we can observe that the accuracy has slightly improve at the cost recall. The hybrid model has better precision, but lower recall compared to the FM Model.
